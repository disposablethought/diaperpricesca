...
/**
 * Scraper for Shoppers Drug Mart
 */
class ShoppersScraper extends BaseScraper {
  ...
  /**
   * Search for diapers on Shoppers Drug Mart
   * @param {Object} params - Search parameters
   * @returns {Promise<Array>} - Array of diaper products
   */
  async searchDiapers(params = {}) {
    try {
      const brands = params.brands || ['Pampers', 'Huggies', 'Life Brand'];
      ...
      for (const brand of brands) {
        for (const size of sizes) {
          ...
          try {
            console.log(`Searching Shoppers Drug Mart for: ${searchQuery}`);
            ...
            $('.product-card').each((i, element) => {
              ...
              // Extract price
              const priceText = $(element).find('.price__value').first().text().trim();
              const price = this.cleanPrice(priceText);
              
              if (!price) return;
              
              // Extract product ID
              const productUrl = $(element).find('a.product-card__link').attr('href') || '';
              const productId = productUrl.split('/').pop() || `shoppers-${Date.now()}-${i}`;
            ...
            console.error(`Error searching Shoppers Drug Mart for ${searchQuery}:`, error.message);
      ...
      console.error('Error in Shoppers Drug Mart scraper:', error);
      return [];
    }
  }
