...
/**
 * Scraper for Canadian Tire with enhanced anti-blocking
 */
class CanadianTireScraper extends BaseScraper {
  ...
  /**
   * Get multiple search URL patterns for Canadian Tire
  ...
  getSearchUrls(searchQuery) {
    ...
    return [
      // Standard search
      `${this.baseUrl}/en/search-results.html?q=${encodedQuery}`,
      // Baby category search
      `${this.baseUrl}/en/search-results.html?q=${encodedQuery}&category=baby`,
      // Sort by relevance
      `${this.baseUrl}/en/search-results.html?q=${encodedQuery}&sort=relevance`,
      // Sort by price low to high
      `${this.baseUrl}/en/search-results.html?q=${encodedQuery}&sort=price-asc`
    ];
  }

  /**
   * Search for diapers on Canadian Tire with enhanced anti-blocking
   * @param {Object} params - Search parameters
   * @returns {Promise<Array>} - Array of diaper products
   */
  async searchDiapers(params = {}) {
    try {
      const brands = params.brands || ['Pampers', 'Huggies', 'Honest Company'];
        ...
        for (const size of sizes) {
          ...
          for (const searchQuery of searchQueries) {
            ...
            for (const url of searchUrls) {
              ...
              try {
                console.log(`Searching Canadian Tire for: ${searchQuery} at ${url}`);
                ...
                if (response.data.includes('blocked') || response.data.includes('captcha')) {
                  console.log('Canadian Tire returned blocking page - trying alternate URL');
                  continue;
                }
                ...
                for (const selector of productSelectors) {
                  ...
                  products.each((i, element) => {
                    ...
                    const titleSelectors = ['.product-name', '.product-title', '.item-name', 'h3', 'h4'];
                    let title = '';
                    ...
                    const priceSelectors = ['.price', '.product-price', '.current-price', '.sale-price'];
                    let priceText = '';
